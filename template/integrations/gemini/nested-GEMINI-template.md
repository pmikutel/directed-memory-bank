# [AREA_NAME] — Gemini CLI area scope

> **Where to put this**: `[AREA_DIR]/GEMINI.md` (e.g. `backend/GEMINI.md`, `frontend/GEMINI.md`, `migrations/GEMINI.md`).
> **Why nested**: Gemini walks the `GEMINI.md` hierarchy automatically (home → ancestors → CWD → subdirs from CWD). A nested file gives area-specific routing without polluting the root entry point.
...
Or use `@` imports to inline them:

@memory-bank/technical/[area-file].md
