# Claude Code Session Essentials
...
- Operation-scoped routing lives in `.claude/skills/` — let the relevant skill fire and load the memory-bank files it names
- Reference files with @memory-bank/[path] when needed; @memory-bank/_index.md is the inventory when you need to discover what knowledge exists
...
Operation-specific context loading is handled by the skills under `.claude/skills/` — each one names the memory-bank files relevant for its operation. When you need to discover what other memory-bank files exist, see @memory-bank/_index.md (the inventory).
...
**Writing into memory-bank/**: follow @memory-bank/project/doc-guide.md — style, length, and hygiene rules. Required reading before any memory-bank edit. The `memory-bank-writer` skill loads it automatically.
